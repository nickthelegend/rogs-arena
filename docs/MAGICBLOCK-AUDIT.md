...
|---|---|---|---|---|---|
| 1 | ER delegation, Player | **GENUINELY USED** | Step 2 `2Hap4Rjf…`, step 4 `2dptWrsB…` (base); later ER txs write both PDAs | `lib.rs:106-117`, `lib.rs:943-951`; `tx.ts:329-349` | PENDING LIVE BROWSER VERIFICATION (not wired) |
...
| 3 | Gasless ER execution | **GENUINELY USED** | Step 6 `41XV3Ute…`: fee payer is session signer `4BZVFzEe…`, which holds **0 lamports on base and on the ER**; `meta.fee = 0` (queried at audit) | `tx.ts:293-308` | PENDING LIVE BROWSER VERIFICATION |
| 4 | Session keys (Gum `SessionTokenV2`) | **GENUINELY USED** | Created: step 3 `4tEVFnMu…`, step 5 `4qiJiSZG…`. Used as signer: steps 6, 7, 9, 10, 12, 13 | `lib.rs:123-127` (`session_auth_or`), `lib.rs:954-975`; `tx.ts:119-130`, `tx.ts:313-324` | PENDING LIVE BROWSER VERIFICATION (not wired) |
| 5 | Pricing oracle read inside the ER | **GENUINELY USED** | Step 16: strike `7725570971918`, close `7726266920786`, outcome YES. First roll `QNfXgdo6…` is in PLAN P2.04 | `lib.rs:483`; `oracle.rs:42-91`; `constants.rs:80-82` | PENDING (chart still Somnia, `chart-btc/index.tsx:17-18`) |
...
| 11 | `undelegate_player` (`commit_and_undelegate`) | **IMPLEMENTED, NOT YET VERIFIED LIVE** | No script or step calls it | `lib.rs:843-853`; `tx.ts:228-233` | PENDING (not wired) |
...
| 21 | BTC-vs-SOL pair rounds | Resolves on relative move of two feeds (SOL/USD `ENYwebBT…`) | Oracle | partial | Market-neutral pairs, a v5 highlight |
