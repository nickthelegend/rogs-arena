/**
 * TEST-PLAN KPR-02: the deployed keeper turns a winning Cheers position into a MagicBlock VRF payout without any
 * player or script sending settle_player or request_cheers. Real devnet + MagicBlock ER + Railway keeper. No mocks.
 *
...
async function main() {
  ...
  record(true, 'P2 buys YES $5 with the Cheers card', `round ${roundId}`, buyCheers.signature)
  ...
  await waitFor(async () => ((await fetchArena(connections.er)).current.id > roundId ? true : null), { timeoutMs: (endTs - now() + 90) * 1000, intervalMs: 2_000, label: 'the crank roll' })
  ...
  appendFileSync(new URL('../../../docs/E2E-RUN.md', import.meta.url), ['', '## Keeper-driven Cheers via MagicBlock VRF (KPR-02)', '', `Run at ${new Date().toISOString()}, round ${roundId}. P1 \`${p1.publicKey.toBase58()}\`, P2 \`${p2.publicKey.toBase58()}\`.`, '', '| Step | Result | Detail | Transaction |', '|---|---|---|---|', ...rows, ''].join('\n'))
